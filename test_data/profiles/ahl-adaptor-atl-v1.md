...
**Status:** adaptor profile, revision 1, released. The released artifact is this document,
byte for byte, published at `https://ahl-protocol.org/profiles/ahl-adaptor-atl-v1.md`, with
its digest alongside at `https://ahl-protocol.org/profiles/ahl-adaptor-atl-v1.md.sha256` (§14).
...
2. **Retrieval by entry id (contract item 4) requires a byte-serving interface that stock ATL
   ...
   not discharge it.
3. **Stock `atl-server` publishes no checkpoint series.** It mints and signs a fresh checkpoint
   per `GET /v1/anchor/:id` — two requests over one root, milliseconds apart, return different
   `checkpoint_time` values and signatures (observed 2026-09-04). The canonical series of §5.2.2,
   the `ITUB` of §5.2.1 and the series-usability of §6.6 therefore have no referent on the
   published surface. A deployment MUST separately publish and retain a canonical checkpoint
   series under §5.2.2 and §16; an independent mirror MAY provide that service (the published
   `ahl-mirror` does, through its checkpoint endpoints — which is not the §10.3 range interface),
   and it is required outside producer control only for L3. This is a deficiency of the
   published log component, recorded here and in §15; §16 stands unchanged.
...
**The cosigned checkpoint object contains exactly `{log_id, tree_size, root_hash,
checkpoint_time, key_id, signature}`** — the six members §6.2 defines — and nothing else
(clarified 2026-09-04, before release; no manifest pinned this profile at the time, so the
text was corrected in place under the same id). `raw` (§6.4), where carried in any receipt-borne
checkpoint — `anchoring.checkpoint`, `later_checkpoint`, a rotation-proof checkpoint alike — is
EXCLUDED from the cosignature preimage; any other checkpoint member is
`invalid` (I-D §7.1 permits `raw` alone as optional). A witness therefore cosigns a typed
six-member projection of the checkpoint, never the JSON object as received, and a verifier
reconstructs the cosigned object from those six members alone. The end-to-end pilot of
2026-09-04 found that a receipt carrying `raw` otherwise changed the preimage and failed every
cosignature; the rule above is what the published witness always computed.
...
- The pair `{ id, digest }` is what a corpus pins. A verifier MUST resolve the profile from
  ...
  artifact held.
- **Release is a precondition for use.** A profile that is not released as an immutable,
  openly published artifact at a stable location has no stable digest, and no manifest may pin
  it: a draft can still change, and a corpus that pinned it would be pinning a moving target.
  Core spec §3 item 6 requires adaptor profiles to be openly published. This document is
  released at `https://ahl-protocol.org/profiles/ahl-adaptor-atl-v1.md`; the digest published
  alongside it, at the same path with the suffix `.sha256`, is a convenience for locating the
  right bytes and never a substitute for recomputation.
...
| item 6 (profile properties) | versioned, immutable, content-addressed, openly published, independently implementable; no knowledge required outside the profile | **satisfied** — versioning, immutability, content addressing and self-containment are settled (§14), this document restates every rule a verifier needs, and the artifact is openly published at the stable location §14 names |
...
10. Retain an outgoing-key checkpoint for every log-key rotation: a deployment performing one
     MUST retain and make available to its witness and to its receipt producer an
    outgoing-key-signed checkpoint of tree size greater than the rotating manifest's entry index,
    together with its inclusion proof, as I-D §7.1's rotation-proof exception requires; the
    witness MUST process that checkpoint under the OUTGOING governance state. The published
    `ahl-mirror` (from 0.3.0) and `ahl-witness` (from 0.4.0) admit such a checkpoint as a distinct
    material class — accepted only under the transition exception, never a series member — and
    serve it: the mirror as the `rotation_proofs[]` element on
    `GET /v1/rotation-proofs/{manifest_entry_index}`, the witness its cosignatures on
    `GET /v1/logs/{log_id}/rotation-cosignatures/{manifest_entry_index}`. (Found missing by the
    end-to-end pilot of 2026-09-04, D7, and closed the same day.)
...
7. **Witness submission and retrieval protocol — a conformance gap.** §11 does not specify how a
   checkpoint, the authenticated entry material and the governance material a witness needs
   reach the witness, nor how a witness obtains them itself; therefore this profile does not yet
   define an independently implementable L3 witness. The published `ahl-witness`'s whole-prefix
   HTTP submission is an implementation-specific, non-normative interface, limited to 2 MiB by
   its default request-body cap, and a delta or pull protocol is not a drop-in replacement: the
   witness re-resolves governance from the full prefix, so any bounded protocol must also carry
   or obtain authenticated governance-state continuity together with a checkpoint-bound
   consistency proof. To be specified before an L3 conformance claim.
